...
void OnTick()
  {
// BATAS WAKTU EXPIRE
   if(TimeCurrent() >= StringToTime("2035.10.6"))
     {
      Alert("EA Expired Contact Telegram @xxxxa.....!!!!");
      return;
     }
               ...
               if(AccountFreeMarginCheck(Symbol(),OP_BUY,GetLots())>0)
                 {
                  ...
                  if(TPLinier)
                    {
                     ModifyTP(0,rata_price(0)+TP_Pip*g_Point);
                    }
                  CloseSell();
                 }
              }
               ...
               if(AccountFreeMarginCheck(Symbol(),OP_SELL,GetLots())>0)
                 {
                  ...
                  if(TPLinier)
                    {
                     ModifyTP(1,rata_price(1)-TP_Pip*g_Point);
                    }
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
     {
      ...
      if(OrderType() == OP_BUY)
        {
         double oTPB = (bepbuy + TP_Pip * Point) * (TP_Pip > 0);
         double oSLB = OrderStopLoss();
         if(SL_BEP_minus > 0)
            oSLB = (bepbuy - SL_BEP_minus * Point);
         if(Buy == 1)
           {
            oTPB = ND(OrderOpenPrice() + TP_Pip   * Point) * (TP_Pip > 0);
            if(StopLoss  > 0)
      ...
      if(OrderType() == OP_SELL)
        {
         double oTPS = (bepsell - TP_Pip* Point) * (TP_Pip> 0);
         double oSLS = OrderStopLoss();
         if(SL_BEP_minus > 0)
            oSLS = (bepsell + SL_BEP_minus * Point);
         if(Sell == 1)
           {
            oTPS = (OrderOpenPrice() - TP_Pip * Point) * (TP_Pip> 0);
            if(StopLoss  > 0)
